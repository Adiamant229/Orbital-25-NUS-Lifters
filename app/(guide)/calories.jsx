...
const calories = () => {
    ...
    const setPALTemp = (x) => {
        setPhysicalActivityLevel(x.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1'));
    }
    ...
    const modeDescription = "Two equations are mainly used to calculate Basal Metabolic Rate (BMR), the energy your body needs to function at a basic level. They are: the Mifflin St-Jeor Equation and the Katch-McArdle Equation. The Mifflin St-Jeor Equation is more commonly used, while the Katch-McArdle Equation is more accurate by using body fat percentages, depending on the accuracy of the method of measuring it. It is also generally more accurate for athletic/lean individuals.";
    const tdeeDescription = "The Total Daily Energy Expenditure, which is an estimation of the energy burned by your body taking into account your physical activity."
    const palDescription = "A factor to be multiplied to your BMR, calculated from your activity during work and leisure time."
    ...
    const mifflinBMRCalc = (sex, weight, height, age) => {
        ...
        sex: 0: female; 1: male
         */
        return Math.round(10 * weight + 6.25 * height - 5 * age + ((sex + 1) % 2) * (-161) + (sex % 2) * 5);
    };
    ...
    return (
        <>
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{width:screenWidth, flex:1}}>
                <ThemedView style={styles.container}>
                    <SafeAreaView style={{flex:1}}>
                        <ScrollView contentContainerStyle={{paddingBottom: 50, paddingTop: 10}}>
                            <ThemedText style={styles.title} title={true}>Calorie Calculator</ThemedText>
                            ...
                            <View style={styles.qnBox}>
                                <View style={{width:120, padding:10, flexDirection:"row"}}>
                                    ...
                                    <Pressable onPress={() => setShowModeDescription(!showModeDescription)}>
                            ...
                            { showModeDescription && <View style={{height:100}}>
                                <ScrollView style={styles.faqBox}>
                                    <ThemedText style={styles.faqTxt}>{modeDescription}</ThemedText>
                                </ScrollView>
                            </View> }
                            <View style={styles.qnBox}>
                            ...
                            <View style={styles.qnBox}>
                                ...
                                <ThemedTextInput keyboardType={"number-pad"} onChangeText={setAgeTemp} value={age} style={styles.textInput}/>
                            </View>
                            <View style={styles.qnBox}>
                                <ThemedText>Height (in cm)</ThemedText>
                                <ThemedTextInput keyboardType={"number-pad"} onChangeText={setHeightTemp} value={height} style={styles.textInput}/>
                            </View>
                            <View style={styles.qnBox}>
                                <ThemedText>Weight (in kg)</ThemedText>
                                <ThemedTextInput keyboardType={"decimal-pad"} onChangeText={setWeightTemp} value={weight} style={styles.textInput}/>
                            </View>
                            {mode === "katch" && (
                                <View style={styles.qnBox}>
                                    <ThemedText>Fat(%)</ThemedText>
                                    <ThemedTextInput keyboardType={"decimal-pad"} onChangeText={setFatTemp} value={fat} style={styles.textInput}/>
                                </View>
                            )}
                            <View style={styles.qnBox}>
                            ...
                            {toggletdee === 1 && (
                                <>
                                    <View style={styles.qnBox}>
                                        <ThemedText style={styles.qnTxt}>Physical Activity Level</ThemedText>
                                        <View style={{flexDirection:"row"}}>
                                            <ThemedTextInput onChangeText={setPALTemp} value={physicalActivityLevel} keyboardType={"decimal-pad"} style={{...styles.textInput, flex:1}}/>
                                            <ThemedButton onPress={()=>setModalVisible(true)} style={{flex:1}}>
                                ...
                                >
                                    <View style={styles.modalBackdrop}>
                                        <View style={styles.modalContainer}>
                                            <ThemedText style={styles.qnTxt}>Work Activity</ThemedText>
                                            <DropDownPicker zIndex={2} items={workActivity} value={workVal} setValue={(callback) => {
                                                setWorkVal(callback);
                                                setErrWork(false);
                                            }} open={open1} setOpen={setOpen1}/>
                                            <ThemedText >{workActivityDescriptions[workVal]}</ThemedText>
                                            {errWork && <ThemedText style={styles.errorTxt}>Please select an activity level</ThemedText>}
                                            <ThemedText style={styles.qnTxt}>Leisure Activity</ThemedText>
                                            <DropDownPicker zIndex={1} items={leisureActivity} value={leisureVal} setValue={(callback) => {
                                                setLeisureVal(callback);
                                                setErrLeisure(false);
                                            }} open={open2} setOpen={setOpen2}/>
                                            <ThemedText >{leisureActivityDescription[leisureVal]}</ThemedText>
                                            {errLeisure && <ThemedText style={styles.errorTxt}>Please select an activity level</ThemedText>}
                                            <ThemedButton style={styles.addButton} onPress={() => {
                                                setModalVisible(false);
                                                if (workVal === 0) {
                                                    setErrWork(true);
                                                }
                                                ...
                                                if (workVal !== 0 && leisureVal !== 0) {
                                                    setPALTemp(calculateActivityLevel(workVal, leisureVal));
                                                    setModalVisible(false);
                                                }
                                            }}>
                                                <ThemedText>Calculate</ThemedText>
                            ...
                            <ThemedButton
                                onPress={() => {
                                    ...
                                    if (mode === "mifflin") {
                                        setBmr(mifflinBMRCalc(sex, tempWeight, tempHeight, tempAge));
                                    } else {
                            ...
                            {showResults && (<>
                                <View style={{flexDirection:"row"}}>
                                    <View style={styles.resultsBox}>
                                        <ThemedText>BMR: {bmr}</ThemedText>
                                    </View>
                                    { toggletdeeres &&
                                    <View style={styles.resultsBox}>
                                        <ThemedText>TDEE: {tdee}</ThemedText>
                                    </View>
                                    }
                                </View>
                            </>)}
